...
{
	int fd;
	// Intentar con device-tree (soporta Ubuntu en Raspberry Pi)
	...
	if (fd >= 0) {
		...
		if (n > 0) {
			...
			// Heurística: contiene "Raspberry Pi"
	...
	for (i = 0; i < (int)sizeof(value_str) - 1; i++) {
		int r = read(fd, &value_str[i], 1);
		if (r <= 0 || value_str[i] == '\n') break;
		if (value_str[i] == 32) break;
...
void GPIO_Config(void)
{
	...
	if(Equipment=='R'){
		...
		printf("Device read failed or unrecognized, defaulting INT_PIN to BCM4.\r\n");
		INT_PIN = 4;
	}
